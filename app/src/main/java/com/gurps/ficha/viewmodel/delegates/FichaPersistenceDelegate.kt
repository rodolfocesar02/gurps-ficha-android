...
class FichaPersistenceDelegate(
    ...
    suspend fun carregarFicha(nomeArquivo: String): Result<Personagem> {
        val json = fichaStorage.carregarFicha(nomeArquivo) 
            ?: return Result.failure(Exception("Arquivo '$nomeArquivo' não encontrado."))
        
        return try {
            val result = PersonagemInterop.importarJson(json)
            Result.success(result.personagem)
        } catch (e: Exception) {
            // Tenta o fallback caso o Interop falhe (pode ser um JSON muito antigo sem envelope)
            runCatching { 
                Personagem.fromJson(json) 
            }.fold(
                onSuccess = { Result.success(it) },
                onFailure = { Result.failure(Exception("Erro ao processar JSON: ${it.message}", it)) }
            )
        }
    }
