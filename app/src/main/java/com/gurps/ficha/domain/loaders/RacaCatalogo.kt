...
data class RacaDefinicao(
    ...
    val mt: Int = 0,
    val secundarios: Map<String, Double> = emptyMap(),
...
object RacaCatalogo {
    ...
    fun resolver(raca: RacaDefinicao, repo: DataRepository): ResultadoResolucao {
        ...
        val modelo = ModeloRacial(
            ...
            modPontosVida = (raca.secundarios["pv"] ?: 0.0).toInt(),
            modVontade = (raca.secundarios["vontade"] ?: 0.0).toInt(),
            modPercepcao = (raca.secundarios["percepcao"] ?: 0.0).toInt(),
            modPontosFadiga = (raca.secundarios["pf"] ?: 0.0).toInt(),
            modVelocidadeBasica = (raca.secundarios["velocidadeBasica"] ?: 0.0).toFloat(),
            modDeslocamentoBasico = (raca.secundarios["deslocamentoBasico"] ?: 0.0).toInt(),
            vantagens = vantagens,
