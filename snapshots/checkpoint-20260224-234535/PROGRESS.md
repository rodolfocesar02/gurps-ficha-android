...
- Armaduras (evolução recente):
  - Lote A: seleção de armaduras com filtros de `NT + Local` e lista com resumo claro (`NT/RD/Peso/Custo`).
  ...
  - Ajuste de fluxo: seleção de armadura passou para `NT + Local` (regra do livro), removendo filtro por peso.
  - Layout da aba Equipamentos reorganizado:
    - `Equipamentos Manuais`, `Armas`, `Escudos`, `Resumo`, `Armaduras` (Armaduras por último).
    - itens de armadura ficam agrupados apenas no card `Armaduras`.
    - visual dos itens de armadura reduzido para `Nome + RD` (menos poluição na lista).
