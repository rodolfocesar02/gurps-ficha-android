...
- Lote 6 (Equipamento e economia) - pacote 8 iniciado:
  - ponto de salvamento criado antes da implementação:
    - `snapshots/checkpoint-20260224-193129`
  - tabela de armaduras convertida para JSON:
    - script: `scripts/convert_armaduras_v1.py`
    - saída: `app/src/main/assets/armaduras.v1.json` (`72` itens, com suporte a componentes em linhas de continuação).
  - integração na aba `Equipamentos`:
    - catálogo de armaduras adicionado no `DataRepository` e `ViewModel`;
    - novo fluxo `Adicionar Armadura` com busca e lista alfabética;
    - ao selecionar armadura, preenche automaticamente custo/peso e registra dados de proteção (`Local/RD`) nas notas do equipamento.
  - validações executadas com sucesso:
    - `.\gradlew.bat :app:compileDebugKotlin testDebugUnitTest --no-daemon`
    - `.\gradlew.bat :app:assembleDebug --no-daemon`
- Lote 6 (Equipamento e economia) - pacote 9 iniciado:
  - tabela de armas de fogo convertida da planilha do usuário:
    - entrada: `tabela de armas de fogo.xlsx`
    - conversor: `scripts/convert_armas_fogo_v1_raw.py`
  - saídas geradas:
    - `app/src/main/assets/armas_fogo.v1.raw.json` (`62` itens)
    - `app/src/main/assets/armas_fogo.v1.normalized.json` (`62` itens)
    - `app/src/main/assets/armas_fogo.v1.review_flags.json` (`25` itens com flags)
  - robustez de pipeline ajustada:
    - células de peso convertidas indevidamente para data (`x/y`) agora são preservadas como `dia/mes`;
    - parser numérico de peso corrigido para aceitar ponto decimal (`6.8`, `7.3`) sem inflar valor.
  - integração na ficha:
    - `DataRepository` passou a carregar armas à distância de dois catálogos:
      - `armas_distancia.v1.normalized.json`
      - `armas_fogo.v1.normalized.json`
    - seleção continua unificada na aba `Equipamentos`, com filtros/busca/ST já existentes.
...
- Lote 3 (Perícias) fechado nesta etapa e validado.
- Lote 4 (Magia) fechado nesta etapa e validado.
- Lote 5 (Combate) fechado nesta etapa (escopo de ficha, sem jogabilidade tática).
- Fontes de produção já forçadas e estáveis:
  - `vantagens.v3.json`
  - `desvantagens.v2.json`
- Pipeline de equipamentos já ativo na ficha:
  - armas corpo a corpo
  - armas à distância (base atualizada por planilha)
  - escudos com integração de `DB` em Bloqueio
  - armaduras com seleção e preenchimento automático em Equipamentos
- Auto-save de recuperação implementado para reduzir perda de trabalho por fechamento inesperado.

## Itens Que Precisam Revisão
- Encoding/mojibake residual em textos vindos de OCR (normalização futura de acentuação).
- Revisão de consistência das tabelas importadas:
  - `armas_corpo_a_corpo.v1.review_flags.json`
  - `armas_distancia.v1.review_flags.json`
  - `armas_fogo.v1.review_flags.json`
- Revisão semântica de custo/peso em itens com múltiplos modos (valores com `/`, `+` e especiais como `var.`/`desp.`).
- Verificação funcional em dispositivo real após lotes grandes de UI (estabilidade e persistência).
- Lote 2 (Vantagens/Desvantagens) segue pausado até entrada final de tags/regras pendentes.
