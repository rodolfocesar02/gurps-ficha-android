...
- **`SilhuetaDoCorpo.kt`** — A silhueta tocável do botão PV (Lote PV-1b), **só na variante visual**. 🔴 O realce é montado com `faixasDaRegiao`, que chama o **mesmo `idEm` do toque** — com duas fontes (um polígono para pintar, uma regra para tocar) bastaria mexer numa para o app pintar o braço e registrar o tronco, com as duas telas parecendo certas. ⚠️ O zoom **move a janela** sobre a mesma imagem; a conta arte→tela mora numa classe só (`Janela`), usada para desenhar e para o toque.
