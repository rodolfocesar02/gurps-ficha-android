...
- **`network/MestreIAClient.kt`**: O mensageiro. Faz a chamada técnica para o servidor (HuggingFace) e extrai o JSON da resposta da IA.
- **`domain/MestreIAUseCase.kt`**: O tradutor. Pega o que a IA escreveu e usa **Busca Fuzzy** (similaridade) para encontrar as perícias e magias reais no banco de dados do App e injetá-las na ficha.
...
1.  **Geração de Personagem**: Fluxo completo de transformar descrição em atributos e habilidades.
2.  **Consulente de Regras**: Tira dúvidas de GURPS enviando o contexto da ficha atual.
