...
### Lote UI-MAGIA-1 — 22 de Julho de 2026 (P12 conjurar em faixas + C11 encolher área + C12 ritual)
**Três regras estavam prontas e testadas no domínio, mas sem gatilho no app — código certo que nunca rodava. Juntadas num lote só de UI, a pedido do usuário, para ele testar UMA vez em vez de três.**
- **P12 — conjurar no modo de FAIXAS.** A conjuração só existia no menu do **token**, que só existe no grid tático: sem grade, um herói mago **não tinha como lançar magia**. O painel de combate ganhou o botão **Conjurar**, reusando o mesmo `SubDialogoConjurar` de dois passos — busca, seletor de energia, "Segurar" projétil e o painel de ritual vêm de graça.
- E a parte que **não era só um botão**: área sem grade não tem hex para mirar. Antes o `iniciarMiraArea` deixava a mira **pendurada para sempre**, esperando um toque que nunca vinha. Novo `resolverAreaPorFaixa`: o centro é o próprio herói e atinge quem estiver no raio — a aproximação honesta que o modelo de faixas permite, já que ali só existe distância ao herói, não posição.
- **C11 — encolher a área.** Lista de "Áreas ativas" com botão de encolher. **Expandir não é oferecido**, porque o livro proíbe; e se algo tentar, o motor recusa com linha no log.
- **C12 — ritual alternativo.** Painel **recolhido por padrão**: a maioria das conjurações é o ritual completo, e quem não abrir não é penalizado. O total aparece sempre no cabeçalho (`Ritual: −4`), porque o número é o que decide — descobrir a penalidade **depois** de errar a jogada seria pior que não ter a opção. O ritual atravessa toda a ponte até o motor e entra no NH como **parcela nomeada**. Caprichar **dobra o tempo** de operação **antes** da redução por NH alto, com teste travando a ordem.
- 🟢 Gate: **912 por variante, 0 falhas**. 🚦 **Toca UI → PARA para teste no aparelho.**
----------------------------------------------------------------------------------------------------------------------------------------------------
...
### Lote AUTOM-2 — 27 de Julho de 2026 (commits e77029ac, c9686caa, ec719e66)
**Trilhos da automação: nota no bônus manual, interpretador de efeitos e Registry das desvantagens (branch GURPS-Saga)**
...
- **Status:** ✅ Build OK nas 2 variantes · gate **1108/0** (+50 testes no loop).
- **Validado no aparelho em 27/07** (commit `540e32da` com as correções): `+2 Pendulear` na Escalada e `+1 Reflexos em Combate` no Sacar Rápido aparecem nas duas abas; Esquiva/Apara com +1; autocontrole rolando; custo de Flashbacks e Avareza corretos. **Dois ajustes**: (1) o card do autocontrole quebrava o "NA 15" na vertical — faltava `weight` na coluna; (2) correção de documentação — são **30** desvantagens com autocontrole, não 35: contei pela palavra na descrição em vez do marcador `*` do custo. O app já usava o critério certo e estava correto ao não oferecer autocontrole em Flashbacks (MB p.141: o custo dele depende da intensidade, não de NA).

### Lote AUTOM-4 — 27 de Julho de 2026 (commits dcd86a5a, fc7a1596, 56d75f22, 052a9fe0)
**Loop autônomo: FASE 3 — os últimos ganchos, e o encerramento honesto do plano (branch GURPS-Saga)**
- **V-3 / V-4** (`dcd86a5a`): bônus de **atributo** declarativo. Cada uma das 9 propriedades calculadas do `Personagem` (ST, DX, IQ, HT, Vontade, Percepção, PV, PF, Velocidade Básica) ganhou **uma linha** chamando `AtributoBonusRules`. O motor não engordou.
  - **A armadilha era recursão infinita**: "PV depende de ST" e uma vantagem que dá "+ST" faz o cálculo de ST pedir o bônus, que pede o ST de novo. Resolvido com uma trava por thread — enquanto um atributo está sendo calculado, ele devolve 0 para si mesmo em vez de se chamar. Analogia: é a mesma coisa que não deixar uma pessoa entrar de novo numa fila em que ela já está.
  - Das 4 vantagens de atributo do catálogo (ST Braçal, DX Braçal, Crescimento, Redução de Tamanho), **nenhuma** entra na soma geral: as duas primeiras valem só para os braços e as duas últimas mudam o Modificador de Tamanho. Foram declaradas com `escopo`, o interpretador as reconhece e **não** as soma. Declarar sem somar não é desperdício — é o registro de que a regra foi lida e classificada.
- **V-5** (`fc7a1596`): bônus **CONDICIONAL**. Este é o que destrava a maioria dos bônus do GURPS: quase todo bônus do livro vem com um "quando". *Rosto Sincero* dá +1 só "ao tentar parecer honesto", não em toda Lábia. O app não tem como saber a intenção do jogador, então **pergunta**: um painel de caixinhas antes de rolar, cada uma dizendo a condição por extenso. Marcadas somam no total.
- **REACAO-1** (`56d75f22`): **Teste de Reação** (MB p.494) — o app nunca teve. O jogador rolava 3d6, consultava a tabela no livro e lembrava de cabeça os modificadores. Agora o total já vem somado, com a lista de onde cada ponto veio e a tabela na tela. Reação é o único teste do GURPS **sem NH**: o modificador desloca o resultado dentro da tabela.
  - **O gate pegou um erro meu**: eu liberei o alvo `"reacao"` no validador Python e esqueci do teste Kotlin. A build quebrou e apontou. É exatamente para isso que os dois existem em espelho.
- **3 lotes DESCARTADOS com motivo** (`052a9fe0`), não adiados — o registro evita que a próxima sessão tente de novo:
  - **V-6 (RD natural)**: `Resistência a Dano` tem nove modificadores que mudam *como* a RD funciona (ablativa, parcial por local, só contra corte). `{"valor": 2}` não diz nada disso. É regra, não dado.
  - **V-7 / D-5 (deslocamento)**: `Voo` e `Deslocamento Aquático` **já têm cálculo próprio** no `Personagem`. Declarar criaria **contagem dupla** — o erro que o Lote M-1 existe para evitar.
  - **D-6 (vulnerabilidade)**: multiplica o dano recebido. Pertence ao `InjuryRules`, no motor de combate — o `TraitRule` não tem gancho para isso, e não deveria ter.
...
### Lote MIRA-2 — distância do alvo no diálogo de mira — 29 de Julho de 2026 (versão 3.7-MIRA2)
Pedido do usuário depois de validar o MIRA-1 no aparelho: *"existe uma regra de acertar alvos com arma à distância, uma regra de a cada 2 metros de distância tem uma penalidade de -1"*.

- ⚠️ **A regra não era essa, e fui atrás no livro antes de codar.** A penalidade **não é linear**: até 2 m não há nenhuma, e daí em diante **cada aumento de ~50% na distância custa −1** (MB p.373 e a Tabela de Tamanho e Velocidade/Distância, p.550-551). Linear daria −50 a 100 metros e ninguém acertaria nada; o certo é **−10**.
- **`TabelaVelocidadeDistancia.kt`** (novo, `domain/rules/`): a coluna do livro escrita à mão, degrau a degrau, para poder ser conferida linha por linha contra a página. Além da tabela impressa, segue a regra de continuação (cada 10× vale mais −6).
- 🔴 **As duas armadilhas, ambas travadas por teste:**
  - **Arredonda para a distância MAIOR.** O exemplo do livro: 17 m vira 20 m = **−6**, não 15 m = −5. Arredondar "para o mais perto" daria um bônus que o personagem não tem, e o número sai plausível na tela — ninguém perceberia.
  - **Velocidade SOMA à distância, não é uma segunda penalidade.** Motoboy a 40 m viajando a 30 m/s conta como 70 → **−9**. Como duas penalidades separadas daria −15, quase o dobro. O rascunho do usuário tinha duas linhas com dois seletores independentes, que levaria exatamente a esse erro; virou **uma conta só, com a soma visível na tela**.
- **O seletor `− / +` anda de DEGRAU da tabela, não de metro em metro.** Assim **cada toque vale exatamente −1** e chegar a 100 m são 10 toques em vez de 98. O botão deixa de ser um contador e passa a ser a própria regra. Há um teste de invariante garantindo que dois degraus vizinhos sempre diferem em 1 — se isso quebrar, o botão vira um contador qualquer.
- **A velocidade começa escondida** atrás de "+ alvo em movimento", porque o próprio livro manda ignorá-la na maioria dos combates a pé.
- ⚠️ **O estado da distância mora na ABA, não no diálogo.** Se morasse no diálogo, sumiria ao fechar e o **toque simples** no NH voltaria a rolar sem ela **sem avisar**. Como fica na aba, vale para os dois gestos — e por isso aparece escrita no card ("alvo a 20m (−6)") e no rótulo da rolagem. Redutor que age sem aparecer é número que ninguém consegue conferir.
- **`AlcanceDoAtaque.kt`** (novo): decide se o ataque é à distância por **duas fontes** — a arma escolhida manda (o catálogo é dividido em três arquivos e `armaTipoCombate` só tem três valores), a perícia é o reserva. As duas juntas cobrem os dois furos: ataque "Arcos" com a fonte de dano ainda em "Dano ST", e faca de arremesso empunhada com a perícia Faca.
- ⚠️ **A regra do "é à distância?" saiu da Saga.** Ela morava em `domain/combat/TraducaoFichaParaCombate.kt`; deixar a ficha perguntando para lá recriaria a amarra que o MIRA-1 tinha acabado de desfazer com o `LocalAtaque`.
- **`PericiasDeCombate.kt`** (novo): `PERICIAS_COMBATE` saiu de `Personagem.kt` (que passava de 1200 linhas) e virou duas listas — corpo a corpo e distância —, com o nome antigo virando a **união**, para não quebrar Apara e Bloqueio. Três perícias estavam no bloco errado por arrumação e vieram para o lado certo: **Sopro**, **Lançador de Lanças** e **Projetor de Pressão**.
- **Dois avisos que a ficha já podia dar e não dava:** *fora de alcance* (Máx) e *além do 1/2D — o dano sai pela metade*. O segundo é o que mais escapa na mesa, porque não muda o ataque, muda o dano. ⚠️ O **arco não tem alcance fixo**: vem como múltiplo da ST (`×10/×15`), e com a ST Braçal ligada é a força dos braços que atira.
- **Testes:** `TabelaVelocidadeDistanciaTest` (12) e `AlcanceDoAtaqueTest` (15), inclusive os dois exemplos numéricos do livro.
- **Status:** ✅ Build OK nas 2 variantes · lint OK · gate verde · ⏭️ **PENDENTE: teste no aparelho** — roteiro T-D1..T-D9.

### 🔴 MIRA-2b — os avisos de alcance não saíam, e o rótulo mentia — 29 de Julho de 2026 (versão 3.8-MIRA2b)
Teste no aparelho do MIRA-2: **T-D1 a T-D7 passaram** (prints do usuário — a linha só aparece no Arco, cada toque vale −1, `50 m + 20 m/s = 70 → −9` correto, o card guardou `alvo a 50m (−9)` e o log saiu `Ataque Arcos a 50m (−9) (NH 9)`). O **T-D8 falhou**: *"isso não apareceu, acho que o sistema não está pegando esta automação"*.

- 🔴 **Causa: o app perguntava "qual arma?" no lugar errado.** A tela lia só a **fonte de dano** escolhida, e o normal é ela ficar em **"Dano ST"** — que é o padrão e era o que estava no print. Com isso a arma vinha nula, o alcance vinha vazio, e nenhum aviso tinha como sair, **mesmo com o arco na ficha e a perícia Arcos selecionada**. Analogia: perguntar com que arma o personagem ataca olhando só para o campo do dano; se o jogador não trocou aquele campo, a resposta era "nenhuma".
- **Conserto — `AlcanceDoAtaque.armaDoAtaque`, uma escada de três degraus:** a arma escolhida na fonte de dano; senão a arma de longe cujo **grupo** casa com a perícia (`ARCO` ↔ Arcos, `Pistola` ↔ Armas de Fogo/Pistola); senão a **única** arma de longe da ficha. Nenhuma das três resolvendo, devolve nulo — avisar com a arma errada é pior que não avisar.
- 🔴 **E um defeito meu de véspera: o rótulo do card mentia.** Ele mostrava só a distância, então com a velocidade ligada aparecia **"alvo a 2m (−2)"** — número que não fecha com a tabela e que o jogador não tem como conferir (2 m sozinho é zero). O −2 vinha do `2 m + 3 m/s = 5`. Agora o rótulo diz **"alvo a 2m + 3m/s (−2)"**, no card e no log.
- ⚠️ **Silêncio é resposta ambígua — e foi o que confundiu.** A arqueira tem **ST 9** com Arco Longo (`×15/×20`), então o 1/2D dela é **135 m**: a 50 m não havia nada a avisar, e a ausência de mensagem foi lida como "não funciona". A linha agora mostra **`1/2D 135 m · Máx 180 m`** o tempo todo, e quando a arma não tem alcance cadastrado (ficha anterior ao Lote 371) **diz isso** em vez de sumir. A pior mensagem é nenhuma.
- **Testes:** +6 casos em `AlcanceDoAtaqueTest` (21 no total), incluindo o exato caso do usuário — achar o arco com a fonte de dano em "Dano ST" — e o caso em que **não pode chutar** (duas armas de longe, nenhuma casando com a perícia).
- **Status:** ✅ Build OK nas 2 variantes · lint OK · gate verde · APKs gerados · ⏭️ **PENDENTE: refazer o T-D8 no aparelho**, agora a partir de 150 m.

### Lote TAL-1 — os dez Talentos, o curinga, e dois defeitos do catálogo — 29 de Julho de 2026 (versão 3.9-TAL1)
Você marcou `(pode fazer)` no Talento. Entrou junto o que era JSON no mesmo molde (itens 2, 3 e 4 do plano).

- 🔴 **Os dez Talentos não faziam nada.** As dez existiam no catálogo **sem um único efeito declarado**: quem comprava Artífice nível 2 gastava 20 pontos e não ganhava um ponto de NH. São ~80 perícias. Agora cada uma tem `porNivel` nas perícias do livro mais **um** bônus de reação **condicional** — o livro condiciona (*"se existir a chance de ela ficar impressionada"*), então virou caixinha, não soma automática.
- 🔴 **Faltava uma vantagem inteira: Agente Cativante.** Ela simplesmente não existia em `vantagens.v3.json`. Entrou com 15 pontos/nível e as 13 perícias sociais. ⚠️ É a única com a condição **invertida**: o bônus de reação **some** se ele estiver tentando manipular a pessoa, e isso está escrito no texto da caixinha.
- 🔴 **Dois defeitos do próprio catálogo, com acento comido:** `Mendicncia` → **Mendicância** e `Analise de Mercado` → **Análise de Mercado**. O segundo saiu de cruzar `pericias.json` com `pericias_v2_rules_map.json`, que tinha a grafia certa. Nome errado ali é bônus mudo **para sempre**, porque o casamento é por nome exato. Entrou teste que varre o catálogo procurando 3+ consoantes seguidas — o sinal de acento perdido.
- ⚠️ **O que quase deu errado, e o que salvou.** O bônus sai de `bonuses[skillName]`, **busca exata por chave de mapa**. E o catálogo é traiçoeiro: `Engenharia/NT` leva barra, `Contabilidade` não; `Conserto de Equipamento Eletrônico/NT (†)` leva barra **e** adaga; `Trato-Social` tem hífen; `Pesca` do livro é **`Pescaria`** no catálogo. Escrevendo os dez, a conferência contra `pericias.json` pegou **seis nomes meus errados** antes de virarem bônus mudo. Analogia: é discar o número certo com o DDD errado — não dá "número inválido", só não chama.
- **Sobreposição: seis perícias, não três.** Eu tinha mapeado Engenharia, Veterinária e Naturalista; o script achou também **Análise de Mercado, Contabilidade e Finanças** (Habilidade Matemática ∩ Perspicácia Comercial). O livro permite passar de +4 nesse caso, e a notinha do NOTA-1 nomeia as duas origens — testado.
- **Teto de 4 níveis** gravado como `max: 4` nas dez (MB p.91).
- **Curinga `*` — "qualquer perícia".** Três vantagens do livro não dão lista, dão **situação**: Toque Sensível (+4 *"qualquer tarefa que utiliza o tato"*), Venturoso (+1 em risco desnecessário) e Versátil (+1 em criatividade). Enumerar as 278 perícias seria absurdo e ficaria errado no dia seguinte. Com o curinga a caixinha aparece em **toda** perícia. ⚠️ **E não vale para defesa nem reação** — o livro fala de *"testes de habilidade"*, e um +1 indevido na Esquiva passaria batido porque parece plausível. Há teste para cada um dos cinco alvos proibidos.
- **Mais dois buracos fechados:** **Carisma** dava só o bônus de reação e o livro nomeia **quatro perícias** (Adivinhação, Liderança, Mendicância, Oratória) — não estavam lá. **Visão Hiperespectral** dava o +3 do teste de Visão mas **nenhuma** das quatro perícias do livro; quem comprava recebia metade do que pagou. Entraram também o **+5 em Perseguição** da Visão 360° e o **+3** da Visão Periférica.
...
### ✅ VALIDADO — T-T, T-L e T-M no aparelho, + 2 correções — 29 de Julho de 2026 (versão 4.6-LUZ2)
**Teste no aparelho com prints do usuário: os 21 itens de T-T (8), T-L (7) e T-M (6) passaram.**

- **Validados**: os dez Talentos entregando bônus (Artífice +2 em Mecânica e Ferreiro, Habilidade Matemática +3 em Matemática); a **sobreposição** funcionando — Engenharia **+5 (Artífice +2, Habilidade Matemática +3)** com as duas origens nomeadas; Agente Cativante na reação com a condição invertida; as três caixinhas do curinga aparecendo em **toda** perícia e **não** aparecendo na reação nem nas defesas; Visão Hiperespectral +3 em Observação; a luz da cena nos três casos (**−7 + Visão Noturna 4 → −3**, **escuridão total −10** sem a Visão Noturna valer, **Visão no Escuro cancela tudo**); o −5 entrando no ataque e na defesa no log; **Voando 11** com Velocidade 5,75 (fração cortada) e **Escalando 6**; Golpe Rápido a **−3** citando Mestre de Armas e depois Treinado por um Mestre; os degraus da apara (**−2** com mestria, **−6** com rapieira na 4ª).
- 🔴 **T-L5 — pergunta do usuário: "confira se teste de perícias devem entrar esse redutor".** Devem, **mas não todas**. O livro amarra a escuridão à **visão**: a Visão Noturna cancela penalidade *"em testes que envolvam a visão ou no combate"* (p.97). Escalar no escuro é mais difícil; lembrar de uma data de História, não. Descontar em **toda** perícia penalizaria Contabilidade e Teologia por causa da luz da sala — número errado em silêncio; descontar em **nenhuma** deixa o jogador subtraindo de cabeça. **Conserto:** a escuridão virou **caixinha** em cada perícia, com o texto *"se este teste depender de ver"*, já com a Visão Noturna descontada. Quem decide é o Mestre — mesmo padrão do Rosto Sincero.
- 🔴 **T-M4 — "quando selecionar 2, eu fiz o apara, mas não caiu pra 0 o redutor, é possível?"** É, e estava faltando. Agora o contador **anda sozinho** depois de cada apara rolada — ele acabou de gastar uma do turno, e a próxima já custa mais —, e há um botão **"Novo turno — zerar"** no painel. O app não tem como saber quando o turno virou, então o zerar é explícito; o que ele podia fazer sozinho (avançar) passou a fazer.
